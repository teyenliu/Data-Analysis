...
class StockerTW():
    # Class members
    url_dict_ = {'TWSE': "http://www.twse.com.tw/exchangeReport/STOCK_DAY?response=html&date={0}&stockNo={1}"}
    columns_dict_ = {'TWSE': 9}
    column_name_dict_ = {'TWSE': ['Date', 'Volume', 'Turnover', 'Open', 'High', 'Low', 'Close', 'Diff', 'Count']}

    # Initialization requires a ticker symbol
    def __init__(self, ticker, exchange='TWSE', year_num=1):
        
        # Symbol is used for labeling plots
        self.symbol = "tw_" + ticker

        # Use TWSE.com.tw web site
        url_pattern = self.url_dict_[exchange]
        date_range = []
        
        # we retrieve year_num year long data from now in TWSE
        self.generateDateRange(year_num)

        # Initialize an empty dataframe
        stock_data = pd.DataFrame(columns=range(0, self.columns_dict_['TWSE']))
        # Retrieval the financial data
        try:
            for date in self.date_range:
                my_url = url_pattern.format(date, ticker)
                print my_url
                tmp_stock_data = self.connectTW_Stock_Source(my_url)
                stock_data = stock_data.append(tmp_stock_data)
                print "wait for 3 seconds to retrieve next data..."
                time.sleep(3)
        except Exception as e:
            print('Error to do Initialization.')
            print(e)
        
        # Setup column name
        stock_data.columns = self.column_name_dict_['TWSE']

        # Change data type to correct one
        stock_data = stock_data.apply(pd.to_numeric, errors='ignore')

        # Set the index to a column called Date
        stock_data['Date'] = stock_data['Date'].map(lambda a: self.replaceTW_to_AD(a))
        ...
        self.prepareParameters(stock_data, year_num)

    def prepareParameters(self, stock_data, year_num):
        # Data assigned as class attribute
        self.stock = stock_data.copy()
        
        # Minimum and maximum date in range
        self.min_date = min(stock_data['Date'])
        self.max_date = max(stock_data['Date'])
        ...
        print('{} Stocker Date: {}'.format(self.symbol,
                                           self.stock))
        print('{} Stocker Initialized. Data covers {} to {}.'.format(self.symbol,
                                                                     self.min_date.date(),
                                                                     self.max_date.date()))
    @classmethod
    def fromCSV(cls, ticker, csv_file):
        # Symbol is used for labeling plots
        self.symbol = "tw_" + ticker
        if isinstance(csv_file, str):
            stock_data = pd.read_csv("2330_tw.csv", header=0, sep='\t', encoding = 'utf-8')
        else:
            raise Exception('Occur error when reading CSV File: %s' % csv_file)
        # Make init() more clear
        self.prepareParameters(stock_data)

    #### Danny Liu's extend implementation ###

    """
    Generate the date range for querying TWSE
    """
    def generateDateRange(self, years):
        self.date_range = []
        for m in range(-12*years, 2):
            self.date_range.append(self.getMonthdelta(datetime.now(), m).strftime('%Y%m%d'))

    """
    Given the delta to modify month in the date
    """
    def getMonthdelta(self, date, delta):
    ...
    """
    Transfer Republic Era (for Taiwan) to A.D. year
    """
    def replaceTW_to_AD(self, tw_date):
        idx = tw_date.find("/")
    ...
    """
    def connectTW_Stock_Source(self, url):
        try:
            ...
            print(e)
        return self.retriveTW_Stock_Data(soup, 9)
    ...
    Return The dataframe of stock data
    """
    def retriveTW_Stock_Data(self, soup, column_size=9):
